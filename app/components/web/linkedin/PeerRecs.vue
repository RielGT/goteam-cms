...
import type { SectionHeaderContent, Rec, RecsSummary, Tone } from "../types";
...
const ringClass = (t: Tone) =>
  ({
    purple: "ring-brand-purple/20",
    green: "ring-brand-green/20",
    orange: "ring-brand-orange/20",
  })[t];
</script>

<template>
  <section :id="sectionId" class="border-y border-zinc-200/70 dark:border-white/5 bg-white/40 dark:bg-white/[0.02] relative overflow-hidden">
    ...
    <div class="relative max-w-6xl mx-auto px-5 sm:px-8 py-24 sm:py-32">
      ...
      <div class="relative">
        <div id="recTrack" class="flex gap-5 overflow-x-auto snap-x snap-mandatory pb-6 -mx-5 px-5 sm:mx-0 sm:px-0 scroll-smooth" style="scrollbar-width: thin;">
          <article
            v-for="r in recs"
            :key="r.name"
            class="rec-card snap-start shrink-0 w-[88%] sm:w-[420px] p-7 rounded-2xl border border-zinc-200 dark:border-white/10 bg-paper dark:bg-ink hover:border-brand-purple transition shadow-soft flex flex-col"
          >
            <header class="flex items-start gap-4 mb-5">
              <img :src="r.avatar" :alt="r.name" class="w-12 h-12 rounded-full ring-2" :class="ringClass(r.ringTone)">
      ...
      <div v-if="summary" class="mt-10 flex flex-wrap items-center justify-between gap-4 reveal">
        <div class="flex items-center gap-3 text-sm text-zinc-600 dark:text-zinc-400">
          <div class="flex -space-x-2">
            <img
              v-for="(a, i) in summary.avatars.slice(0, 4)"
              :key="i"
              :src="a"
              class="w-7 h-7 rounded-full border-2 border-paper dark:border-ink"
              alt=""
            >
            <div v-if="summary.avatars.length > 4" class="w-7 h-7 rounded-full border-2 border-paper dark:border-ink bg-brand-purple text-white text-[10px] font-mono grid place-items-center">
              +{{ summary.avatars.length - 4 }}
            </div>
          </div>
          <span><span class="font-semibold text-ink dark:text-white">{{ summary.count }} {{ summary.label }}</span></span>
        </div>
        <a
          :href="summary.cta.href"
          :target="summary.cta.external ? '_blank' : undefined"
          :rel="summary.cta.external ? 'noopener' : undefined"
          class="inline-flex items-center gap-2 text-sm font-medium px-5 py-2.5 rounded-full border border-zinc-200 dark:border-white/10 hover:border-brand-purple hover:text-brand-purple transition"
        >
